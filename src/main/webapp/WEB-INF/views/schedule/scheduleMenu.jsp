  ...
  <title>scheduleMenu.jsp</title>
	...
	<script>
		...
		function scheduleInputView() {
			let scheduleInput = '<div id="scheduleInputForm">';
			...
			scheduleInput += '<option vlaue="모임">모임</option>';
			scheduleInput += '<option vlaue="업무">업무</option>';
			scheduleInput += '<option vlaue="학습">학습</option>';
			scheduleInput += '<option vlaue="여행">여행</option>';
			scheduleInput += '<option vlaue="기타">기타</option>';
		...
		// 스케줄 수정하기
		function updateCheck(idx, content, part) {
			...
			scheduleInput += '<option vlaue="'+part+'" selected>'+part+'</option>';
			if(part!='모임'){
				scheduleInput += '<option vlaue="모임">모임</option>';
			}
			if(part!='업무'){
...
<div class="container">
  ...
  <div>
  	...
  	<input type="button" value="등록창닫기" onclick="scheduleInputHidden()" id="scheduleInputHidden" class="btn btn-danger mb-2" />
  	<input type="button" value="돌아가기" onclick="location.href='${ctp}/schedule/schedule?yy=${fn:split(ymd,'-')[0]}&mm=${fn:split(ymd,'-')[1]-1}';" class="btn btn-info mb-2" />
  </div>
  <div id="demo"></div>
  <hr/>
  			...
  			<tr>
  				...
  				<td>
  					<a href="#" onclick="modalView('${vo.part}','${fn:replace(vo.content,newLine,'<br/>')}')" data-toggle="modal" data-target="#myModal">
	  					...
	  					<c:if test="${fn:indexOf(vo.content,newLine) == -1}">${fn:substring(vo.content,0,21)}</c:if>
  					</a>
  				</td>
  				<td>${vo.part}</td>
  				<td>
  					<input type="button" value="수정" onclick="updateCheck('${vo.idx}','${fn:replace(vo.content,newLine,'<br/>')}','${vo.part}')" id="scheduleUpdateOpen${vo.idx}" class="btn btn-warning" />
